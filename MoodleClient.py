...
class MoodleClient(object):
    ...
    def upload_file_draft(self,file,progressfunc=None,args=(),tokenize=False):
            print("paso 1")
            file_edit = f'{self.path}/user/edit.php'
            #https://eduvirtual.uho.edu.cu/user/profile.php
            resp = self.session.get(file_edit,proxies=self.proxy)
            print('paso 2')
            soup = BeautifulSoup(resp.text, 'html.parser')
            ...
            callback = partial(progrescall)
            monitor = MultipartEncoderMonitor(encoder,callback=callback)
            resp2 = self.session.post(post_file_url,data=monitor,headers={"Mozilla/5.0 (Windows NT 10.0; Win64; x64)" : "AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.72 Safari/537.36"},proxies=self.proxy)
            of.close()
            print('paso 4')
            data = self.parsejson(resp2.text)
            print(data)
